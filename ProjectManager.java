...
/**
 * ProjectManager is responsible for managing projects and interacting with the
...
public class ProjectManager {
  ...
  /**
   * Adds a new project to the database, including the foreign key IDs for
   * people involved.
   *
   * @param connection the database connection
  ...
  public static void addProject(Connection connection, Project project) {
    String query =
        "INSERT INTO Project (project_name, building_type, physical_address, "
        + "total_fee, deadline, is_finalised, architect_id, engineer_id, "
        + "manager_id, customer_id) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, ?)";
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
      ...
      pstmt.setInt(7, project.getStructuralEngineerId());
      pstmt.setInt(8, project.getProjectManagerId());
      pstmt.setInt(9, project.getCustomerId());
  ...
  public static void updateProject(Connection connection, Project project) {
    String query =
        "UPDATE Project SET project_name = ?, building_type = ?, "
        + "physical_address = ?, total_fee = ?, deadline = ?, architect_id = "
        + "?, engineer_id = ?, manager_id = ?, customer_id = ? "
        + "WHERE project_id = ?";
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
      pstmt.setString(1, project.getProjectName());
      pstmt.setString(2, project.getBuildingType());
  ...
  public static void finalizeProject(Connection connection, int projectId) {
    String query =
        "UPDATE Project SET is_finalised = 1, completion_date = ? WHERE "
        + "project_id = ?";
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
  ...
  public static void findProjectByIdOrName(Connection connection, String input) {
    String query =
        "SELECT * FROM Project WHERE project_id = ? OR project_name LIKE ?";
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
      if (input.matches("\\d+")) {
  ...
  public static void findPastDueProjects(Connection connection) {
    String query =
        "SELECT * FROM Project WHERE deadline < CURDATE() AND is_finalised = 0";
    try (PreparedStatement pstmt = connection.prepareStatement(query);
         ResultSet rs = pstmt.executeQuery()) {
      ...
      System.out.println("An error occurred while retrieving past due projects.");
    }
  }

  /**
   * Retrieves a person's ID by their name from the database.
   *
   * @param connection The database connection
   * @param personType The type of person (e.g., Architect, Engineer, etc.)
   * @param personName The name of the person
   * @return The person's ID if found, otherwise -1
   */
  public static int getPersonIdByName(Connection connection, String personType,
                                      String personName) {
    String tableName = "";
    String columnName = "";

    // Determine table and column names based on person type
    switch (personType.toLowerCase()) {
      ...
      case "customer":
        ...
        break;
      default:
        System.out.println("Invalid person type: " + personType);
        return -1;
    }
    ...
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
      ...
      try (ResultSet rs = pstmt.executeQuery()) {
        if (rs.next()) {
          return rs.getInt(1);  // Return the person's ID
        } else {
          System.out.println(personType + " with name " + personName
              + " not found.");
        }
      }
    } catch (SQLException e) {
      logger.log(Level.SEVERE, "Error retrieving " + personType
          + " ID by name.", e);
    }
